...
cd "$BACKEND_DIR"
nohup env \
  YOLO_PYTHON="$YOLO_PYTHON_CMD" \
  ASSISTANT_GEMMA_OLLAMA_URL="$GEMMA_OLLAMA_URL" \
  ASSISTANT_GEMMA_MODEL="$GEMMA_MODEL" \
  ASSISTANT_GEMMA_FALLBACK_MODEL="$GEMMA_FALLBACK_MODEL" \
  "$JAVA_CMD" -jar "$BACKEND_JAR" >> "$BACKEND_LOG" 2>&1 &
